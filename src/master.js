...
function stopWorker(worker) {
    console.log(chalk.yellow('stopping', worker.process.pid));
    worker.disconnect();
...
cluster.on('online', function (worker) {
    console.log(chalk.green('Worker ' + worker.process.pid + ' is now online'));
});

cluster.on('exit', function (worker, code, signal) {
    console.log(chalk.white.bgRed('Worker ' + worker.process.pid + ' died: ' + (code || signal)));
...
process.on('SIGHUP', function () {
    console.log(chalk.green('restarting all workers'));
...
console.log(chalk.gray('Starting initial workers. All ' + workerCount + ' of them'));
forkNewWorkers();
console.log(chalk.gray('app master', process.pid, 'booted'));
